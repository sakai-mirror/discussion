/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/discussion/branches/sakai_2-2-x/discussion-impl/impl/src/java/org/sakaiproject/discussion/impl/DbDiscussionService.java $
 ...
 * </p>
 * <p>
 * The sql scripts in src/sql/chef_discussion.sql must be run on the database.
 * </p>
 */
public class DbDiscussionService extends BaseDiscussionService
{
	/** Our logger. */
	private static Log M_log = LogFactory.getLog(DbDiscussionService.class);

	/** The name of the db table holding discussion channels. */
	protected String m_cTableName = "DISCUSSION_CHANNEL";

	/** The name of the db table holding discussion messages. */
	...
	public void init()
	{
		try
		{
			...
			if (m_autoDdl)
			{
				m_sqlService.ddl(this.getClass().getClassLoader(), "sakai_discussion");
			}
	...
	/**
	 * fill in the draft and owner db fields, and also catagory and reply to
	 */
			...
			m_sqlService.dbRead(connection, sql, null, new SqlReader()
			{
				...
				public Object readSqlResultRecord(ResultSet result)
				{
					try
					{
						...
						DiscussionMessage m = new BaseDiscussionMessageEdit(null, root);

						// pick up the fields
						String owner = m.getHeader().getFrom().getId();
						boolean draft = m.getHeader().getDraft();
						String category = m.getDiscussionHeader().getCategory();
						...
						if (!ok)
							M_log.info("convertToDraftPlus: channel: " + channelId + " message: " + messageId + " owner: " + owner
									+ " draft: " + draft + " category: " + category + " replyTo: " + replyTo + " ok: " + ok);

						count++;
						if (count % 100 == 0)
						{
							M_log.info("convertToDraftPlus: " + count);
						}
						return null;
					}
